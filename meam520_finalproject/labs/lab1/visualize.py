...
def show_all_FK(state):
    ...
    try:
        ...
        joints, T0e, T_temp = fk.forward(q)
        show_pose(T_temp,"progress_frame")
...
configurations = [
    ...
    np.array([ 0,    pi/4, -pi/2, -pi/4,  pi/2, pi,   -pi/4 ]),
]
    ...
    if sys.argv[1] == 'FK':
        ...
        for i, q in enumerate(configurations):
            ...
            if i < len(configurations) - 1:
                final_config = arm.get_positions()
                jointPositions, _ = fk.forward(final_config)
                print("Final Position: {}".format(jointPositions[7]))
